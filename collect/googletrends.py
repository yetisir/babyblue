...
def load_google_trends(coin, from_date, to_date, sample_interval=1,
    ...
    for i in range(num_intervals):
        ...
        int_start_str = interval_start.strftime(time_format)
        int_end_str = interval_end.strftime(time_format)
        timeframe = int_start_str + ' ' + int_end_str

        cached_file_name = ('google-trends_{coin}_{from_date}_to_'
                            '{to_date}.pkl').format(coin=coin,
                                                    from_date=int_start_str,
                                                    to_date=int_end_str)
        ...
        if len(coin_df) > 0:
            if coin_df.index[-1] - coin_df.index[0] > overlap_interval:
                previous_interval_overlap = coin_df.loc[(interval_start -
                                                         overlap_interval):,
                                                        coin]
                current_interval_overlap = interval_df.loc[:(interval_start +
                                                             overlap_interval),
                                                           coin]
                previous_average = previous_interval_overlap.mean()
                current_average = current_interval_overlap.mean()

                interval_df.loc[:, coin] = (interval_df[coin] *
                                            previous_average / current_average)
        coin_df = coin_df.append(interval_df.loc[(interval_start +
                                                  overlap_interval):])
