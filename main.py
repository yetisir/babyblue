...
if __name__ == '__main__':
    ...
    assimilator = DataAssimilator(start_date, end_date, 'BTC')
    gtrends_columns = assimilator.add_google_trends(
        filters=[GaussianFilter(),
                 NotchFilter()])
    reddit_columns = assimilator.add_reddit_comments(
        filters=[DailyAverage()])
    fourchan_columns = assimilator.add_fourchan_comments(
        filters=[DailyAverage()])
    ...
    file = py.plot(
        data[binance_columns+['total_comments']].iplot(
            asFigure=True, secondary_y=binance_columns),
